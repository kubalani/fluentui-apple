...
@objc public enum MSFAvatarPresence: Int, CaseIterable {
    ...
    func color(isOutOfOffice: Bool) -> Color {
        ...
        case .doNotDisturb:
			color = NSColor.red
        case .offline:
			color = isOutOfOffice ? NSColor.gridColor : NSColor.darkGray
        case .unknown:
			color = NSColor.white
        }
...
@objc public class MSFAvatarState: NSObject, ObservableObject {
    @objc @Published public var image: NSImage?
    @objc @Published public var primaryText: String?
    @objc @Published public var secondaryText: String?
    @objc @Published public var ringColor: NSColor?
    @objc @Published public var backgroundColor: NSColor?
    @objc @Published public var foregroundColor: NSColor?
...
public struct AvatarView: View {
    ...
    public var body: some View {
		...
		let avatarImage: NSImage? = ((style == .outlined || style == .outlinedPrimary) ? NSImage.init(named: "person_48_regular") :
										(shouldUseDefaultImage ? NSImage.init(named: "person_48_filled") : state.image))
        let avatarImageSizeRatio: CGFloat = (shouldUseDefaultImage) ? 0.7 : 1

        let avatarContent = (avatarImage != nil) ?
			AnyView(Image(nsImage: avatarImage!)
                        .resizable()
        ...
        let accessibilityLabel: String = {
            ...
            return (state.isOutOfOffice ? "OOF" : defaultAccessibilityText)
        }()
...
@objc open class MSFAvatar: NSObject {

    @objc open var view: NSView {
    ...
    @objc public init(style: MSFAvatarStyle = .default,
        ...
        self.avatarview = AvatarView(style: style,
                                           size: size)
        self.hostingController = NSHostingController(rootView: AnyView(avatarview))
